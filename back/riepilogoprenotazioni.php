...
var tf11 = setFilterGrid("table11", table11_Props);
    ...
    <body>
        <?php
        $stringaSql= 'select prenotazione.id as id,concat(substr(data,1,4),"-", substr(data,5,2) ,"-", substr(data,7,2)) as datanormale, nome as nome,cognome as cognome ,pickup.data as data from prenotazione,utente,pickup where utente.id=prenotazione.idutente and prenotazione.pickupa=pickup.id ORDER BY prenotazione.id ASC';
	...
	url:'<?php echo Utils::getBase() ?>json/jsonFromSql.php?sql=select prenotazione.id as id,concat(substr(data,1,4),"-", substr(data,5,2) ,"-", substr(data,7,2)) as datanormale, nome as nome,cognome as cognome ,pickup.data as data from prenotazione,utente,pickup where utente.id=prenotazione.idutente and prenotazione.pickupa=pickup.id ORDER BY prenotazione.id ASC',
	dataType: 'json', 
